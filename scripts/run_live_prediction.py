...
def run_prediction():
    ...
    print("Récupération des vols Aviationstack...")
    flights_as = collector.get_live_flights()
    print(f"{len(flights_as)} vols AS récupérés")
    print("Récupération des vols AirLabs...")
    flights_al = collector.get_live_flights_airlabs()
    print(f"{len(flights_al)} vols AL récupérés")
    flights = flights_as + flights_al
    print(f"{len(flights)} vols au total")
    print("Récupération des météos...")
    weather = collector.collect_weather_for_flights(flights)
    print(f"{len(weather)} villes météo récupérées")
